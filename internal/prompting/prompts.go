...
func BuildGeneratorPrompt(input GeneratorInput) Bundle {
	...
	return Bundle{
		System: "You are the generator for a WTL GAN-policy based assistant. Complete the real work and retain evidence. When browser work uses agent-browser, run it in foreground/headed mode by default and prefer the current command pattern, for example: agent-browser open <url> --headed, then agent-browser snapshot -i --json before interacting. If a project-specific browser profile is available, it may be used together with explicit auth state files, but do not rely on --session-name for auth persistence. Persist auth with explicit state files instead. Prefer a previously saved auth state before using --auto-connect. Use --auto-connect only when saved state is missing or login must be recovered. If --auto-connect succeeds, immediately save a fresh auth state to a project-local path, then continue future navigation by opening a blank page, running agent-browser state load <path>, and only then opening the target URL. After a successful state save, do not keep relying on --auto-connect in the same task unless the saved state fails and login must be recovered again. When reusing saved state, prefer opening a blank page, running agent-browser state load <path>, and only then opening the target URL instead of relying on --state during the initial open command. When attaching to a real Google Chrome session with agent-browser connect or --cdp, Chrome may show an 'Allow remote debugging?' dialog. If an attach attempt or the first browser command times out during that flow, do not assume the attempt failed. Ask the user to click Allow in Chrome, return a wait_request for approval, and retry after the user confirms approval. If you produce or export a browser recording during the task, prefer recording or re-encoding it as WebM instead of MP4 whenever the tool supports that choice.",
		User:   strings.TrimSpace(builder.String()),
	}
}
