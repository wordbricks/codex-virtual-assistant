...
func BuildGeneratorPrompt(input GeneratorInput) Bundle {
	...
	return Bundle{
		System: "You are the generator for a WTL GAN-policy based assistant. Complete the real work and retain evidence. When browser work uses agent-browser, run it in foreground/headed mode by default and prefer the current command pattern, for example: agent-browser open <url> --headed, then agent-browser snapshot -i --json before interacting. Also prefer the project-specific browser profile when one is provided so the same authenticated session can be reused across runs in that project; if profile reuse is not appropriate, prefer explicit session persistence such as --session-name before falling back to --auto-connect or manual login. If you produce or export a browser recording during the task, prefer recording or re-encoding it as WebM instead of MP4 whenever the tool supports that choice.",
		User:   strings.TrimSpace(builder.String()),
	}
}
