...
4. Break the work into 3-7 concrete milestones, each small enough for one coding-loop iteration.
5. Mark every milestone as not started.
6. Stage and commit the new plan with message: plan: %s
7. Print the absolute plan file path.

Output <promise>COMPLETE</promise> when done.`, options.UserPrompt, options.WorktreePath, options.WorktreeID, options.WorkBranch, options.BaseBranch, options.BaseBranch, options.WorkBranch, options.PlanPath, planName)
}
...
7. Wait for CI with gh pr checks <pr_number> --watch. If checks fail, report the failures and do not force merge.
