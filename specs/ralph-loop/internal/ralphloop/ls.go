...
func executeListCommand(runCtx runContext) int {
	sessions, err := listRunningRalphSessions(runCtx.repoRoot, runCtx.command.ListOptions.Selector)
	if err != nil {
	...
	pages := paginateItems("ls", items, runCtx.command.Common)
	switch runCtx.command.Common.Output {
	case OutputText:
		if err := runListCommand(runCtx.repoRoot, runCtx.command.ListOptions, runCtx.stdout); err != nil {
			return writeCommandError(runCtx.stdout, runCtx.stderr, runCtx.command.Common.Output, string(runCtx.command.Kind), err)
		}
		return 0
	case OutputNDJSON:
