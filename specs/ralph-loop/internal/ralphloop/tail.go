...
func executeTailCommand(runCtx runContext) int {
	if runCtx.command.TailOptions.Follow && runCtx.command.Common.Output == OutputNDJSON {
		logPath, err := findLatestRalphLog(runCtx.repoRoot, runCtx.command.TailOptions.Selector)
		if err != nil {
			return writeCommandError(runCtx.stdout, runCtx.stderr, runCtx.command.Common.Output, string(runCtx.command.Kind), err)
		}
		if err := followLogNDJSON(runCtx.ctx, logPath, runCtx.command.TailOptions.Lines, runCtx.stdout); err != nil {
			return writeCommandError(runCtx.stdout, runCtx.stderr, runCtx.command.Common.Output, string(runCtx.command.Kind), err)
		}
		return 0
	}
	if runCtx.command.Common.Output == OutputText {
		if err := runTailCommand(runCtx.ctx, runCtx.repoRoot, runCtx.command.TailOptions, runCtx.stdout, runCtx.stderr); err != nil {
			return writeCommandError(runCtx.stdout, runCtx.stderr, runCtx.command.Common.Output, string(runCtx.command.Kind), err)
		}
		return 0
	}
	...
	lines, err := readTailLines(logPath, runCtx.command.TailOptions.Lines)
	if err != nil {
		return writeCommandError(runCtx.stdout, runCtx.stderr, runCtx.command.Common.Output, string(runCtx.command.Kind), err)
	}
...
func readTailLines(path string, lines int) ([]string, error) {
	body, err := readTailFile(path, lines)
	if err != nil {
		return nil, err
	}
	if body == "(log file is empty)" {
		return []string{}, nil
	}
...
func followLogNDJSON(ctx context.Context, path string, lines int, stdout io.Writer) error {
	command := exec.CommandContext(ctx, "tail", "-n", fmt.Sprintf("%d", lines), "-f", path)
	pipe, err := command.StdoutPipe()
	if err != nil {
		return err
