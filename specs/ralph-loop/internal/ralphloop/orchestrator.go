...
func runMain(ctx context.Context, repoRoot string, options MainOptions, stdout io.Writer, stderr io.Writer) (result mainRunResult, err error) {
	_, _ = fmt.Fprintln(stdout, "Resolving worktree and branch naming")
	naming := resolveNamingFn(ctx, options, repoRoot, spawnCodexClient)
	result.NamingSource = naming.Source
	result.NamingReason = naming.Reason
	...
	result.WorktreeID = worktree.WorktreeID
	result.WorktreePath = worktree.WorktreePath
	result.WorkBranch = worktree.WorkBranch
	result.BaseBranch = worktree.BaseBranch
	result.RuntimeRoot = worktree.RuntimeRoot
	result.Preserved = options.PreserveWorktree
	...
	result.PRURL = buildPrCreatedEvent(prOutput).PRURL
