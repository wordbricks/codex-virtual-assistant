...
- Milestone 3 completed:
  - Reused existing `Client.StreamEvents` + `streamSSE` path for TUI mode (no new execution/transport engine).
  - Updated `streamRunTUI` to open the SSE stream and pass it into a TUI message adapter.
  - Added typed Bubble Tea stream messages (`tuiStreamEventMsg`, `tuiStreamErrMsg`, `tuiStreamClosedMsg`) and a channel-backed `waitForTUIStreamMsg` command loop.
  - Added `streamRunEventsForTUI` goroutine bridge that:
    - reads SSE events with existing parser,
